...
@app.route('/api/ai/generate-practice', methods=['POST'])
@require_auth
def generate_practice(current_user):
    ...
    if not language:
        return jsonify({"detail": "language is required"}), 400

    prompt_context = f"the user has learned about: {', '.join(topics)}" if topics else "the user is a beginner."
    prompt = f"Based on the following context about what the user has watched: '{prompt_context}', generate a single, specific {language} coding practice question. Return ONLY a JSON object with two keys: 'title' (a short 2-4 word title) and 'text' (the detailed problem description)."
    ...
    try:
        if genai_client:
            ...
            return jsonify({"title": practice_data.get("title", f"{language} Practice"), "text": practice_data.get("text", "Please practice basic features.")}), 200
        else:
            return jsonify({"detail": "AI features not available - API key not configured"}), 500
    except Exception as e:
        print(f"Error generating practice: {e}")
        return jsonify({"detail": "Failed to generate practice"}), 500


@app.route('/api/ai/rag-process-video', methods=['POST'])
@require_auth
def rag_process_video_endpoint(current_user):
    data = request.get_json()
    video_id = data.get("video_id")
    transcript = data.get("transcript")
    
    if not video_id or not transcript:
        return jsonify({"detail": "video_id and transcript are required"}), 400
    ...
    try:
        ...
        return jsonify({"message": "Video queued for RAG AI processing seamlessly!"}), 200
    except Exception as e:
        print(f"Error in rag_process_video: {e}")
        return jsonify({"detail": "Failed to queue video for RAG processing"}), 500
